...
local function GetNearbyHeli(playerPos)
    ...
    for _, vehicle in pairs(game.Workspace.Vehicles:GetChildren()) do
        ...
        -- Check if helicopter is within 50 studs
        if dist <= 50 then
...
local function GetVehiclePos(playerPos)
    ...
    -- First check for nearby helicopters within 50 studs
    local nearbyHeli = GetNearbyHeli(playerPos)
    if nearbyHeli then
        return nearbyHeli.Seat.Position, nearbyHeli
    end

    -- If no nearby helicopter, proceed with original logic
    for _, vehicle in pairs(game.Workspace.Vehicles:GetChildren()) do
        if vehicle.name ~= 'Heli' then continue end
        if vehicle.Seat.position.y > 300 then continue end
        local pos = vehicle.Seat.Position
